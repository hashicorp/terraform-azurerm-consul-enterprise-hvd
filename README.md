# HashiCorp Validated Design (HVD) — Consul Enterprise on Azure Virtual Machines

This Terraform module deploys an HVD-aligned Consul Enterprise cluster on Microsoft Azure.
...
For a reference deployment of the prerequisite resources, see the [examples section](#examples).

## Examples

The [`examples/prereqs` directory](./examples/prereqs/README.md) within this repository contains a reference deployment of the aforementioned prerequisite resources.

The [`examples/consul` directory](./examples/consul/README.md) contains a reference root-level module which uses the `prereqs` state output to deploy this module.

## Deployment

Upon initial deployment, Consul servers will auto-join and form a fresh cluster. The ACL system is always enabled in deny-by-default mode. When `consul_agent.bootstrap_acls` is true (the default setting), the cluster is [bootstrapped](#acl-bootstrapping) with basic policies and ACL tokens generated. An operator, or other automation, should then connect to the cluster to perform any post-deployment customization.

Azure Key Vault is required with this module. See: [Secrets Management](#secrets-management) for further information.

## ACL Bootstrapping

When the [`consul_agent.bootstrap_acls`](#consul-agent-configuration) parameter is set to true cloud-init will attempt to:
...
This module performs all required software installation and configuration at boot time via cloud-init. Currently, a dpkg-based Linux distribution with the `apt` suite is expected. Support for RPM-based distributions is planned for a later release. All testing is performed against Ubuntu 22.04 LTS.
